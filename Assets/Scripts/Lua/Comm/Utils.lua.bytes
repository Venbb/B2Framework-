...
--[[--
    计算表格包含的字段数量
    Lua table 的 "#" 操作只对依次排序的数值下标数组有效，table.size() 则计算 table 中所有不为 nil 的值的个数。
]]
...
-- @param string self 输入字符串
-- @param string sep 分割标记字符或字符串
-- @return fields#fields  包含分割结果的数组
...
--------------------------------
-- 去掉字符串首尾的空白字符，返回结果
-- @function [parent=#string] trimall
-- @param string self 输入字符串
-- @return string#string  结果

-- end --
function string.trimall(self)
    return self:gsub("[ \t\n\r]", "")
    -- return self:gsub("%s*(.-)%s*", "%1")
end
...
--------------------------------
-- 字符串是否为空，返回结果
-- @function [parent=#string] isNilOrEmpty
...
-- end --
function string.startwith(self, pattern, plain)
	plain = plain or true
	local index, _ = self:find(pattern, 1, plain )
	return index == 1
end
...
--------------------------------
-- 是否以某个字符串结束，返回结果
-- @function [parent=#string] endwith
-- @param string self 输入字符串
-- @return bool#bool  结果

-- end --
function string.endwith(self, pattern, plain)
    plain = plain or true
	local _, index = self:find(pattern, -#self, plain)
	return index == #self
end
-- start --
