...
namespace shaco.Base
{
    ...
    {
        public delegate void CALL_FUNC_EVENT(object sender);
        ...
        public CallBackInfo this[int index]
        {
            get
            {
                ...
                {
                    Log.Exception("EventCallBack this[int index] error: out of range, index=" + index + " count=" + _callBacks.Count);
                    return null;
                }
                else
                {
        ...
        public void AddCallBack(object defaultSender, CALL_FUNC_EVENT callfunc, bool invokeOnce = false)
        {
            lock (_callBacks)
            {
                ...
                {
                    Log.Exception("EventCallBack AddCallBack error: has invalid parameters" + GetCallBackInformation(callfunc));
                }

                var callbacksTmp = GetCallBacks(defaultSender, callfunc, true);
                if (!callbacksTmp.IsNullOrEmpty())
                {
                    Log.Exception("EventCallBack AddCallBack error: has added callback by defaultSender=" + defaultSender.ToTypeString()
        ...
        public CallBackInfo[] RemoveCallBack(object defaultSender, CALL_FUNC_EVENT callfunc)
        {
            lock (_callBacks)
            {
                ...
                {
                    Log.Exception("EventCallBack RemoveCallBack error: not find callback by defaultSender=" + defaultSender.ToTypeString()
        ...
        public void InvokeAllCallBack(object defaultSender)
        {
            InvokeAllCallBackBase(defaultSender);
        }

        public void InvokeAllCallBack()
        {
            InvokeAllCallBackBase(null);
        }

        public EventCallBack Clone()
        {
            var retValue = new EventCallBack();
        ...
        private void InvokeAllCallBackBase(object sender)
        {
            lock (_callBacks)
            {
                if (_isInvoking)
                ...
                for (int i = 0; i < _callBacks.Count; ++i)
                {
                    if (null == _callBacks)
                    {
                        Log.Exception("EventCallBack invokeAllCallBack erorr: _callBacks is null !");
                        break;
                    }

                    if (i >= _callBacks.Count)
                    {
                        Log.Exception("EventCallBack invokeAllCallBack erorr: index out of range ! index=" + i + " callback count=" + _callBacks.Count);
                        break;
                    }
                    ...
                    {
                        Log.Exception("EventCallBack invokeAllCallBack erorr: callback information is null !");
                        continue;
                    }
                    ...
                    if (null == callbackInfoTmp.DefaultSender)
                    {
                        ...
                        Log.Exception("EventCallBack invokeAllCallBack error: sender is missing, we will remove it" + GetCallBackInformation(callbackInfoTmp.CallFunc));
                    }
                    ...
                    if (null != callbackInfoTmp.CallFunc)
                    {
                        if (!callbackInfoTmp.WillRemove)
                        {
                            callbackInfoTmp.CallInvokeEventStack.StartTimeSpanCalculate("EventCallBack Invoke: target=" + callbackInfoTmp.CallFunc.Target + " method=" + callbackInfoTmp.CallFunc.Method);
                            {
                                try
                                {
                                    callbackInfoTmp.CallFunc(sender == null ? callbackInfoTmp.DefaultSender : sender);
                                }
                                catch (System.Exception e)
                                {
                                    Log.Exception("EventCallBack invoke error: target=" + callbackInfoTmp.CallFunc.Target + " method=" + callbackInfoTmp.CallFunc.Method + " e=" + e);
                                }
                            }
                    ...
                    {
                        Log.Warning("EventCallBack invokeAllCallBack warning: callback function is missing" + GetCallBackInformation(callbackInfoTmp.CallFunc), GetInvokeSender(sender, callbackInfoTmp));
                    }
