...
namespace shaco.ExcelDataReader.Core
{
    ...
    internal class CommonWorkbook
    {
        ...
        /// </summary>
        public Dictionary<int, NumberFormatString> Formats { get{return _Formats;} }
        public Dictionary<int, NumberFormatString> _Formats = new Dictionary<int, NumberFormatString>();

        /// <summary>
        /// Gets the the dictionary of mappings between format index in the file and key in the Formats dictionary.
        /// </summary>
        private Dictionary<int, int> FormatMappings { get{return _FormatMappings;} }
        private Dictionary<int, int> _FormatMappings = new Dictionary<int, int>();

        private List<ExtendedFormat> ExtendedFormats { get {return _ExtendedFormats;} }
        private List<ExtendedFormat> _ExtendedFormats = new List<ExtendedFormat>();

        public int GetExtendedFormatCount() { return ExtendedFormats.Count; }
        ...
        {
            int formatIndex;
            if (FormatMappings.TryGetValue(formatIndexInFile, out formatIndex))
            {
                return formatIndex;
            }
