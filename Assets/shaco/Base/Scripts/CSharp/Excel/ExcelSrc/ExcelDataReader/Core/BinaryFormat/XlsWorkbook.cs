...
using shaco.ExcelDataReader.Core.OfficeCrypto;
using shaco.ExcelDataReader.Exceptions;
...
namespace shaco.ExcelDataReader.Core.BinaryFormat
{
    ...
    internal class XlsWorkbook : CommonWorkbook, IWorkbook<XlsWorksheet>
    {
        ...
        public List<XlsBiffRecord> Fonts { get {return _Fonts;} } 
        private List<XlsBiffRecord> _Fonts = new List<XlsBiffRecord>();

        public List<XlsBiffBoundSheet> Sheets { get{return _Sheets;} }
        private List<XlsBiffBoundSheet> _Sheets = new List<XlsBiffBoundSheet>();
        ...
        public int ResultsCount {get{return null != Sheets ? Sheets.Count : -1; }}

        public static bool IsRawBiffStream(byte[] bytes)
        {
            if (bytes.Length < 8)
            {
                throw new ArgumentException("Needs at least 8 bytes to probe", bytes.ToStringArray());
            }
