...
namespace shaco.ExcelDataReader.Core.CsvFormat
{
    ...
    internal class CsvParser
    {
        ...
        private StringBuilder ValueResult { get{return _ValueResult;} set{_ValueResult=value;} }
        private StringBuilder _ValueResult = new StringBuilder();
 
        private List<string> RowResult { get{return _RowResult;} set{_RowResult=value;}}
        private List<string> _RowResult  = new List<string>();

        private List<List<string>> RowsResult { get{return _RowsResult;} set{_RowsResult=value;} }
        private List<List<string>> _RowsResult = new List<List<string>>();
        ...
        public void ParseBuffer(byte[] bytes, int offset, int count, out List<List<string>> rows)
        {
            while (count > 0)
            {
                ...
                bool completed;
                Decoder.Convert(bytes, offset, count, CharBuffer, 0, CharBuffer.Length, false, out bytesUsed, out charsUsed, out completed);
