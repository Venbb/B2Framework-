...
namespace shaco.ExcelDataReader.Core.OpenXmlFormat
{
    internal class XlsxWorkbook : CommonWorkbook, IWorkbook<XlsxWorksheet>
    {
        ...
        public List<XlsxBoundSheet> Sheets { get{return _Sheets;} }
        public List<XlsxBoundSheet> _Sheets = new List<XlsxBoundSheet>();

        public XlsxSST SST { get{return _SST;} }
        public XlsxSST _SST = new XlsxSST();
        ...
        private void ReadCellXfs(XmlReader reader)
        {
            ...
            while (!reader.EOF)
            {
                ...
                {
                    int xfId, numFmtId;
                    int.TryParse(reader.GetAttribute(AXFId), out xfId);
                    int.TryParse(reader.GetAttribute(ANumFmtId), out numFmtId);
