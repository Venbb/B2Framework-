...
using shaco.ExcelDataReader.Log;
...
*/
        ...
        {
            NumberFormatString fmtString = null;
            if (Workbook.Formats.TryGetValue(numberFormatIndex, out fmtString))
            {
                return fmtString;
            }
        ...
        private void GetBlockSize(int startRow, out int blockRowCount, out int minOffset, out int maxOffset)
        {
            ...
            {
                XlsRowOffset rowOffset = null;
                if (RowOffsetMap.TryGetValue(startRow + i, out rowOffset))
                {
        ...
        private IEnumerable<Row> ReadWorksheetRows(XlsBiffStream biffStream)
        {
            ...
            {
                int blockRowCount, minOffset, maxOffset;
                GetBlockSize(rowIndex, out blockRowCount, out minOffset, out maxOffset);
        ...
        private Row EnsureRow(XlsRowBlock result, int rowIndex)
        {
            ...
            if (!result.Rows.TryGetValue(rowIndex, out currentRow))
            {
                ...
                XlsRowOffset rowOffset;
                if (RowOffsetMap.TryGetValue(rowIndex, out rowOffset) && rowOffset.Record != null)
                {
        ...
        private void ReadWorksheetGlobals()
        {
            using (var biffStream = new XlsBiffStream(Stream, (int)DataOffset, Workbook.BiffVersion, null, Workbook.SecretKey, Workbook.Encryption))
            {
                ...
                if (header != null || footer != null)
                {
                    ...
                    {
                        OddHeader = null != header ? header.GetValue(Encoding) : string.Empty,
                        OddFooter = null != footer ? footer.GetValue(Encoding) : string.Empty
                    };
                }
        ...
        {
            XlsRowOffset rowOffset;
            if (!RowOffsetMap.TryGetValue(rowIndex, out rowOffset))
            {
