...
namespace shacoEditor
{
    public class ExportPackageWindow : EditorWindow
    {
        ...
        private ExportPackageWindow _currentWindow = null;
        static private Rect _rectWindow = new Rect();
        private Vector2 _vec2ScrollPosition = new Vector2();

        [MenuItem("shaco/Other/ExportUnityPackage", false, (int)(int)ToolsGlobalDefine.MenuPriority.Other.EXPORT_PACKAGE_1)]
        static ExportPackageWindow OpenExportDialog()
        {
            var retValue = EditorHelper.GetWindow<ExportPackageWindow>(null, true, "ExportPackage") as ExportPackageWindow;
            retValue.Show();
        ...
        [MenuItem("shaco/Other/ExportUnityPackage(Previous) %#p", false, (int)(int)ToolsGlobalDefine.MenuPriority.Other.EXPORT_PACKAGE_2)]
        ...
        void OnEnable()
        {
            _currentWindow = EditorHelper.GetWindow<ExportPackageWindow>(this, true, "ExportPackage") as ExportPackageWindow;
        }
        ...
        void SaveConfig()
        {
            ...
            var fullPathTmp = shaco.Base.FileHelper.GetFullpath("ExportPackageWindowConfig.json");
            shaco.Base.FileHelper.WriteAllByUserPath(fullPathTmp, jsonWrite);
        }

        void LoadConfig()
        {
            var fullPathTmp = shaco.Base.FileHelper.GetFullpath("ExportPackageWindowConfig.json");
            var jsonRead = shaco.Base.FileHelper.ReadAllByUserPath(fullPathTmp);
        ...
        string[] GetAssetsDeep(List<string> listFolder)
        {
            ...
            for (int i = 0; i < listFolder.Count; ++i)
            {
                ...
                shaco.Base.FileHelper.GetSeekPath(listFolder[i], ref listSeek);
