...
namespace shacoEditor
{
    public class LocalizationReplacePrefab : shacoEditor.ILocalizationReplace
    {
        ...
        //属性名字
        private List<string> _propertyPaths = new List<string>() { "mText" };
        ...
        /// </summary>
        virtual public void GetAllLanguageString(string importPath, System.Action<List<shaco.Base.Utility.LocalizationCollectnfo>> callbackCollectInfo, params string[] collectExtensions)
        {
            //获取要收集的组建类型
                    ...
                    if (null != prefabTmp)
                    {
                        ...
                        if (!labelComponentsTmp.IsNullOrEmpty())
                        {
                            for (int j = 0; j < labelComponentsTmp.Length; ++j)
                            {
                                ...
                                //添加到收集路径信息
                                object value = ChangeComponentDataHelper.GetSerializedPropertyValue(labelComponentsTmp[j], _propertyPaths.ToArray());
                                if (null != value)
                                {
        ...
        /// </summary>
        virtual public void RepalceLanguageString(string path, List<shaco.Base.Utility.LocalizationExportInfo> exportInfos)
        {
            //获取要收集的组建类型
            ...
            {
                var internalPath = exportInfos[i].parameter;
                ...
                {
                    //可能存在同名child不同component的情况，再从父类节点重新查找一次
                    var parentTmp = prefab.transform.Find(internalPath.RemoveLastPathByLevel(1));
                    
                    if (null != parentTmp)
                    {
                        var childNameTmp = shaco.Base.FileHelper.GetLastFileName(internalPath);
                        for (int j = parentTmp.transform.childCount - 1; j >= 0; --j)
                        {
                            var childSub = parentTmp.transform.GetChild(j);
                            if (childSub.name == childNameTmp)
                            {
                                componentTmp = childSub.GetComponent(componentType);
                                if (null != componentTmp)
                                    break;
                            }
                        }
                ...
                var oldValue = ChangeComponentDataHelper.GetSerializedPropertyValue(componentTmp, _propertyPaths.ToArray());
                if (null != oldValue)
                {
                    ...
                    ChangeComponentDataHelper.SetSerializedPropertyValue(componentTmp, autoValueTmp, _propertyPaths.ToArray());
                }
            }
        }
        ...
        /// 绘制编辑器
        /// </summary>
        virtual public void DrawInspector()
        {
            _componentTypeFullName = EditorGUILayout.TextField("Component Name", _componentTypeFullName);
            GUILayoutHelper.DrawStringList(_propertyPaths, "Property Name");
        }

        /// <summary>
        /// 是否只在主线程下替换文件
        /// </summary>
        virtual public bool OnlyReplaceInMainThread() { return true; }
        ...
        private string GetPrefabInternalPath(GameObject target)
        {
            ...
            {
                retValue = retValue.Remove(retValue.Length - shaco.Base.FileDefine.PATH_FLAG_SPLIT_STRING.Length, shaco.Base.FileDefine.PATH_FLAG_SPLIT_STRING.Length);
            }
