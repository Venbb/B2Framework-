...
{
    public class LocalizationReplaceWindow : EditorWindow
    {
        //特殊符号转义符表，以防止编码解析问题
        private readonly string[,] TRANSFER_FLAGS = new string[2, 2]
        {
            {" ", "##1"},
            {"\t", "##2"}
        };
        ...
        private LocalizationReplaceWindow _currentWindow = null;
        [SerializeField]
        private string _importCollectCharactersFolder = string.Empty;
        [SerializeField]
        private string _exportCollectCharacetersFolder = string.Empty;
        [SerializeField]
        private shaco.Base.Utility.LanguageType _collectLanguage = shaco.Base.Utility.LanguageType.SimpleChinese;
        ...
        private shaco.Instance.Editor.TreeView.WindowSplitter _lineSeparator = new shaco.Instance.Editor.TreeView.WindowSplitter();


        [MenuItem("shaco/Tools/LocalizationReplace", false, (int)(int)ToolsGlobalDefine.MenuPriority.Tools.LOCALIZATION_REPLACE)]
        static void OpenLocalizationReplaceWindow()
        {
            EditorHelper.GetWindow<LocalizationReplaceWindow>(null, true, "LocalizationReplace");
        }

        void OnEnable()
        {
            _currentWindow = EditorHelper.GetWindow<LocalizationReplaceWindow>(this, true, "LocalizationReplace");
        ...
        void OnGUI()
        {
            ...
            _lineSeparator.BeginLayout(true);
            {
            ...
            _lineSeparator.EndLayout();

            _lineSeparator.BeginLayout();
            {
        ...
        {
            shaco.GameHelper.datasave.WriteString("LocalizationReplaceWindow+_importCollectCharactersFolder", _importCollectCharactersFolder);
            shaco.GameHelper.datasave.WriteString("LocalizationReplaceWindow+_exportCollectCharacetersFolder", _exportCollectCharacetersFolder);
            if (null != _localizationReplaceInterface)
            {
                shaco.GameHelper.datasave.WriteString("LocalizationReplaceWindow+_localizationReplaceInterface", _localizationReplaceInterface.ToTypeString());
            }
            shaco.GameHelper.datasave.WriteArray("LocalizationReplaceWindow+_collectionFileExtensions", _collectionFileExtensions.ToArray());
            shaco.GameHelper.datasave.WriteString("LocalizationReplaceWindow+_localizationReplaceInterface", _localizationReplaceInterface.ToTypeString());
        }

        private void LoadSettings()
        {
            _importCollectCharactersFolder = shaco.GameHelper.datasave.ReadString("LocalizationReplaceWindow+_importCollectCharactersFolder");
            _exportCollectCharacetersFolder = shaco.GameHelper.datasave.ReadString("LocalizationReplaceWindow+_exportCollectCharacetersFolder");
            var localizationReplaceInterfaceNameTmp = shaco.GameHelper.datasave.ReadString("LocalizationReplaceWindow+_localizationReplaceInterface");
            ...
            _collectionFileExtensions = shaco.GameHelper.datasave.ReadListString("LocalizationReplaceWindow+_collectionFileExtensions", new string[] { ".cs" }.ToList());
        }
        ...
        private void DrawExportLanguage()
        {
            GUILayout.BeginVertical("box");
            {
                ...
                //需要收集语言信息的文件后缀名
                GUILayoutHelper.DrawStringList(_collectionFileExtensions, "File Extension");
                ...
                GUILayout.BeginVertical("box");
                {
                    ...
                    GUILayout.BeginVertical("box");
                    {
                        _collectLanguage = (shaco.Base.Utility.LanguageType)EditorGUILayout.EnumPopup("Language", _collectLanguage);
                    }
        ...
        private void DrawImportLanguage()
        {
            GUILayout.BeginVertical("box");
            {
                ...
                GUILayout.BeginVertical("box");
                {
                    GUILayoutHelper.DrawStringList(_externalImportSplitFlag, "External Import Split Flags");
                }
                GUILayout.EndVertical();
        ...
        /// </summary>
        static public void DrawFlags(string prefix, List<shaco.Base.Utility.LocalizationCollectPairFlag> flags, string[] inputFlags)
        {
            if (2 != inputFlags.Length)
            {
                Debug.LogError("LocalizationReplaceWindow DrawFlags erorr: input flag not a pair");
                return;
            }
        ...
        /// </summary>
        private void CollectAndExportLanguage(string importPath, string exportPath, shaco.Base.Utility.LanguageType language)
        {
            if (string.IsNullOrEmpty(exportPath))
            {
                Debug.LogError("LocalizationReplaceWindow CollectAndExportLanguage erorr: invalid export path");
                return;
            }
        ...
        /// <param name="language">需要收集的语言类型</param>
        /// </summary>
        private void CollectAndExportLanguage(List<shaco.Base.Utility.LocalizationCollectnfo> importInfo, string exportPath, shaco.Base.Utility.LanguageType language)
        {
            if (importInfo.IsNullOrEmpty())
            {
                Debug.Log("LocalizationReplaceWindow CollectAndExportLanguage: import information is empty");
            ...
            shaco.Base.Coroutine.ForeachAsync(importInfo, (object value) =>
            {
                try
                {
                    ...
                    for (int i = 0; i < lineString.Length; ++i)
                    {
                        ...
                        switch (language)
                        {
                            case shaco.Base.Utility.LanguageType.SimpleChinese:
                                {
                                    selectLineCharacters = shaco.Base.Utility.SelectSimpleChineseCharacter(stringTmp, _collectFlags);
                                    break;
                                }
                            case shaco.Base.Utility.LanguageType.TraditionalChinese:
                                {
                                    selectLineCharacters = shaco.Base.Utility.SelectTraditionalChineseCharacter(stringTmp, _collectFlags);
                                    break;
                                }
                            case shaco.Base.Utility.LanguageType.Japanese:
                                {
                                    selectLineCharacters = shaco.Base.Utility.SelectJapaneseCharacter(stringTmp, _collectFlags);
                                    break;
                                }
                            case shaco.Base.Utility.LanguageType.English:
                                {
                                    selectLineCharacters = shaco.Base.Utility.SelectEnglishCharacter(stringTmp, _collectFlags);
                                    break;
                                }
                            default: Debug.LogError("LocalizationReplaceWindow CollectAndExportLanguage erorr: unsupport language type=" + language); shouldBreak = true; break;
                        }
            ...
            , (float progress) =>
            {
                ...
                if (progress >= 1.0f)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Debug.Log("LocalizationReplaceWindow CollectAndExportLanguage success=" + success + " collect language count=" + collectLanguageCount + " path=" + exportPath);
                            EditorUtility.ClearProgressBar();
                        }, (float progress2) =>
                        {
        ...
        /// </summary>
        private void ImportAndReplaceLanguage(string tranditionalPath, shaco.Base.Utility.LanguageType language, List<string> defaultCollectFileExtensions)
        {
            //从外部文件中获取翻译信息
                ...
                else
                {
                    ...
                    }, (float progress) =>
                    {
                        ...
                        if (progress >= 1.0f || hasError)
                        {
                            ...
                            AssetDatabase.SaveAssets();
                            EditorHelper.SaveCurrentScene();
                            Debug.Log("LocalizationReplaceWindow ImportAndReplaceLanguage success=" + !hasError);
                        }
                    });
                }
            });
        ...
        /// <summary>
        /// 检查文件是否为excel，如果是excel文件自动序列化为csv格式的文件用于读取
        /// <param name="filePath">文件路径</param>
        /// <return>自动临时创建的csv格式文件路径，如果返回空则表示没有自动创建临时文件</return>
        /// </summary>
        private string CheckAndCreateNewFileFromExcel(string filePath)
        {
            var retValue = string.Empty;

            if (shaco.Base.ExcelHelper.IsExcelFile(filePath))
            {
                var savedCsvPath = shaco.Base.FileHelper.ReplaceLastExtension(filePath, ".txt");
                var excelData = new shaco.Base.ExcelData();
                excelData.Init(filePath);
                var savedSheetsPath = excelData.SaveAsTxt(savedCsvPath);
                if (savedSheetsPath.IsNullOrEmpty())
                    Debug.LogError("LocalizationReplaceWindow CheckAndCreateNewFileFromExcel erorr: can't parse excel, path=" + filePath);
                else
                {
                    if (savedSheetsPath.Length != 1)
                    {
                        Debug.LogError("LocalizationReplaceWindow CheckAndCreateNewFileFromExcel error: only support first sheet, path" + filePath);
                    }
                    else 
                    {
                        retValue = savedSheetsPath[0];
                    }
                }
            }
        ...
        /// </summary>
        private void CollectLanguageInfoFromExternalFile(string tranditionalPath, shaco.Base.Utility.LanguageType language, System.Action<Dictionary<string, List<shaco.Base.Utility.LocalizationExportInfo>>> callbackEnd)
        {
            ...
            {
                shaco.Base.FileHelper.GetSeekPath(tranditionalPath, ref allFiles, false);
            }, () =>
            {
                shaco.Base.Coroutine.ForeachAsync(allFiles, (object value) =>
                {
                    ...
                    var newPath = CheckAndCreateNewFileFromExcel(path);
                    if (!string.IsNullOrEmpty(newPath))
                        path = newPath;

                    string fileNameWithoutExtensions = shaco.Base.FileHelper.RemoveLastExtension(shaco.Base.FileHelper.GetLastFileName(path));
                    if (!fileNameWithoutExtensions.Contains(languageString))
                    {
                        Debug.LogWarning("LocalizationReplaceWindow CollectLanguageInfoFromExternalFile warning: not traditional file, language=" + languageString + "\npath=" + path);
                        return true;
                    }
                    ...
                    try
                    {
                        ...
                        {
#if UNITY_EDITOR_WIN
                            readStringTmp = readStringTmp.Replace("\r\n", "\n");
#endif
                            ...
                            for (int i = lineStrings.Length - 1; i >= 1; --i)
                            {
                                ...
                                {
                                    Debug.LogWarning("LocalizationReplaceWindow CollectLanguageInfoFromExternalFile error: not enough file col, split col=" + splitStrings.Count + " max need col=" + maxNeedCol + " lineString=" + lineString + "\npath=" + path);
                                }
                                else
                                {
