...
namespace shacoEditor
{
    public class LocalizationReplaceDefault : ILocalizationReplace
    {
        ...
        public void GetAllLanguageString(string importPath, System.Action<List<shaco.Base.Utility.LocalizationCollectnfo>> callbackCollectInfo, params string[] collectExtensions)
        {
            ...
            {
                Debug.LogError("LocalizationReplaceWindow CollectAndExportLanguage erorr: invalid param");
                return;
            }
            ...
            shaco.Base.ThreadPool.RunThreadSafeCallBack(() =>
            {
                ...
                //过滤shaco框架中的文本内容
                var shacoBaseRelativePath = shaco.Base.GlobalParams.GetShacoFrameworkRootPath().Remove(applicationPathTmp).ContactPath("Base");
        ...
        public void RepalceLanguageString(string path, List<shaco.Base.Utility.LocalizationExportInfo> exportInfos)
        {
            ...
            {
                Debug.LogError("LocalizationReplaceWindow RepalceLanguageString error: not found path=" + path);
            }
            //替换对应文本
            else
            {
                for (int i = exportInfos.Count - 1; i >= 0; --i)
                {
                    ...
                    {
                        Debug.LogWarning("LocalizationReplaceWindow RepalceLanguageString error: not found orginal text=" + infoTmp.textOriginal + "\npath=" + path);
                    }
                }
