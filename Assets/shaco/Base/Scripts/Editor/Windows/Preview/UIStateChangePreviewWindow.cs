...
{
    [ExecuteInEditMode]
    public class UIStateChangePreviewWindow : EditorWindow
    {
        ...
        private UIStateChangePreviewWindow _currentWindow = null;

        [MenuItem("shaco/Viewer/UIStateChangePreview " + ToolsGlobalDefine.MenuPriority.ViewerShortcutKeys.UI_STATE_CHANGE, false, (int)ToolsGlobalDefine.MenuPriority.Viewer.UI_STATE_CHANGE)]
        static void OpenUIManagerWindow()
        {
            var retValue = EditorHelper.GetWindow<UIStateChangePreviewWindow>(null, true, "UIStateChangePreview");
            retValue.Init();
        }

        void OnEnable()
        {
            _currentWindow = EditorHelper.GetWindow(this, true, "UIStateChangePreview");
            _currentWindow.Init();
        }
        ...
        private void DrawAllUIChangedInfo()
        {
            ...
            for (int i = 0; i < allUIStateChangedInfoTmp.Count; ++i)
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    EditorGUILayout.LabelField("Name", changedInfoTmp.uiKey);
                    ...
                    if (GUILayout.Button(changedInfoTmp.eventType + changedInfoTmp.stackLocationUI.GetPerformanceDescription(), GUILayout.Width(200)))
                    {
                        shaco.Log.Info(changedInfoTmp.stackLocationUI.GetTotalStackInformation());
                        EditorHelper.OpenAsset(changedInfoTmp.stackLocationUI.GetStackInformation(), changedInfoTmp.stackLocationUI.GetStackLine());
                    }
                }
                GUILayout.EndHorizontal();
            }
        ...
        private Object GetScriptAssetFromCache(string uiKey, shaco.UIPrefab uiPrefab)
        {
            if (!_uiStateBindScriptAssets.ContainsKey(uiKey))
                _uiStateBindScriptAssets.Add(uiKey, UIPreviewWindow.GetScriptAssetFromUI(uiKey, uiPrefab));
