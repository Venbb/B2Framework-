    ...
    public partial class BehaviourTreePreviewWindow : EditorWindow
    {
        ...
        private void ShowContextMenu(bool newOnly)
        {
            ...
            for (int i = 0; i < allBehaviourTreeTypes.Length; ++i)
            {
                ...
                menuTmp.AddItem(new GUIContent("Add(" + objTmp.displayName + ")"), false, 
                                (object context) => 
                                {
                                    var attributeTmp = typeTmp.GetAttribute<shaco.Base.BehaviourProcessTreeAttribute>();
                                    objTmp.editorAssetPathTree = null != attributeTmp ? attributeTmp.path : string.Empty;
            ...
            if (!newOnly)
            {
                menuTmp.AddItem(new GUIContent("Remove(Delete)"), false, (object context) =>
                {
                    ExecuteRemove();
                }, null);
                ...
                if (_currentSelectTrees.Count == 1)
                {
                    menuTmp.AddItem(new GUIContent("RemoveChildren"), false, (object context) =>
                    {
                        _currentSelectTrees[0].RemoveChildren(); _updateFormatPositionsDirty = true;
                    }, null);
                }
                menuTmp.AddItem(new GUIContent("ResetScript"), false, (object context) =>
                {
                    for (int i = _currentSelectTrees.Count - 1; i >= 0; --i)
                    {
                        _currentSelectTrees[i].processTypeName = string.Empty;
                        _currentSelectTrees[i].editorAssetProcess = null;
                        _currentSelectTrees[i].editorAssetPathProcess = null;
                    }
                }, null);
            }
        ...
        private void ShowChangeTreeTypeMenu(shaco.Base.BehaviourTree tree)
        {
            ...
            for (int i = 0; i < allBehaviourTreeTypes.Length; ++i)
            {
                ...
                {
                    menuTmp.AddItem(new GUIContent(objTmp.displayName), true, null, null);
                }
                else 
                {
                    menuTmp.AddItem(new GUIContent(objTmp.displayName), false,
                                (object context) =>
                                {
                                    var attributeTmp = typeTmp.GetAttribute<shaco.Base.BehaviourProcessTreeAttribute>();
                                    tree.editorAssetPathTree = null != attributeTmp ? attributeTmp.path : string.Empty;
                                    shaco.Base.Utility.CopyPropertiesAndFields(tree, objTmp);
                                    tree.Replace(objTmp);
                                    _updateFormatPositionsDirty = true;
                                }, null);
                }
                ...
                if (SelectBehaviourTree(curEvent))
                {
                    ...
                    for (int i = 0; i < _currentDragTrees.Count; ++i)
                    {
                        ...
                        else
                        {
                            ...
                            if (null != dragTree.parent && parentTree.parent == dragTree)
                            {
                                parentTree.RemoveMe(true);
                                dragTree.parent.InsertChild(parentTree, dragTree.GetSiblingIndex());
                            }
