...
{
    public class BehaviourTreeRunTimePreviewWindow : EditorWindow
    {
	    ...
	    private BehaviourTreeRunTimePreviewWindow _currentWindow = null;
        ...
        [MenuItem("shaco/Viewer/BehaviourTreeRunTimePreview " + ToolsGlobalDefine.MenuPriority.ViewerShortcutKeys.BEHAVIOUR_RUNTIME_TREE, false, (int)ToolsGlobalDefine.MenuPriority.Viewer.BEHAVIOUR_RUNTIME_TREE)]
        static private void OpenBehaviourTreeRunTimePreviewWindow()
		{
            EditorHelper.GetWindow<BehaviourTreeRunTimePreviewWindow>(null, true, "BehaviourTreeRunTimePreview");
		}

		void OnEnable()
		{
            _currentWindow = EditorHelper.GetWindow(this, true, "BehaviourTreeRunTimePreview");
            _currentWindow.Init();
        }
		...
		private void DrawCurrentRootTress()
		{
			...
			if (string.IsNullOrEmpty(_searchRooTreeName))
			{
                ...
                {
                    DrawVaribles(iter.Key, iter.Value);
                }
			}
			else 
			{
		...
		private void DrawVaribles(Object obj, List<RootTreeInfo> treeInfo)
		{
            if (GUILayoutHelper.DrawHeader(obj.name, obj.name, () => EditorGUILayout.ObjectField(obj, obj.GetType(), true)))
            {
                foreach (var iter2 in treeInfo)
                {
                    EditorGUI.BeginDisabledGroup(!iter2.rootTree.IsRunning());
                    {
                        if (GUILayout.Button(iter2.variableName))
                        {
                            _currentRootTree = iter2;
                            BehaviourTreePreviewWindow.OpenBehaviourTreePreviewWindow(_currentRootTree.rootTree);
                        }
                    }
                    ...
                    GUILayout.BeginVertical("box");
                    {
                        ...
                        _rightWindowScrollPosition = GUILayout.BeginScrollView(_rightWindowScrollPosition);
                        {
                            ...
                            else
                            {
                                ...
                                foreach (var iter in parameters)
                                {
                                    if (null == iter || !iter.ToString().ToLower().Contains(searchLowerName))
                                        continue;
		...
		{
            var treeViewer = EditorHelper.FindWindow<BehaviourTreePreviewWindow>();
            if (null != treeViewer)
            {
                treeViewer.Close();
            }
