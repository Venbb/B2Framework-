    ...
    public partial class BehaviourTreePreviewWindow : EditorWindow
	{
        ...
        private void FormatTreePosition(shaco.Base.BehaviourTree tree, int index, int level)
        {
            ...
            if (null != parent)
            {
                ...
                drawPosition.y = parent.editorDrawPosition.y + parent.editorDrawPosition.height + parent.editorDrawPosition.height / 5 + MIN_ITEM_MARGIN.y;
        ...
        private void FixFormatTreePosition(shaco.Base.BehaviourTree left, shaco.Base.BehaviourTree right)
        {
            ...
            var offsetPosX = maxRightTree.editorDrawPosition.x - maxLeftTree.editorDrawPosition.x + maxRightTree.editorDrawPosition.width + MIN_ITEM_MARGIN.x;
        ...
        private void FixFormatTreeSizeWithGUILayout(shaco.Base.BehaviourTree tree, float minWidth = 0, float minHeight = 0)
		{
			if (Event.current.type == EventType.Repaint)
            {
                ...
                if (this._isShowMiniUI)
                {
                    tree.editorDrawPosition.width = minWidth;
                }
            }
        }
