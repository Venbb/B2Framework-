...
namespace shacoEditor
{
    ...
    public class CustomEnumsField
    {
        ...
        {
            _customEnums = defaultValues.ToList();
        ...
        public bool SetEnum(string enumString)
        {
            ...
            {
                if (onEnumWillChangeCallBack != null)
                    onEnumWillChangeCallBack();
                currentSelectIndex = _customEnums.IndexOf(enumString);
                if (onEnumChangedCallBack != null)
                    onEnumChangedCallBack();
                return true;
            }
        }
        ...
        public void SetEnumDisplay(string[] enumsDisplay)
        {
            ...
            _customEnums = _customEnumsDisplay.ToList();
        }
        ...
        public void DrawEnums(string prefix = "Enum:")
        {
            GUILayout.BeginHorizontal();
            {
                ...
                switch (_currentSelectMode)
                {
                    ...
                    case SelectMode.Remove:
                        {
                            ...
                            if (!string.IsNullOrEmpty(_currentInputEnumString) && GUILayout.Button("OK", GUILayout.Width(SUB_BUTTON_WIDTH / 2 - subButtonOffset)))
                            {
                                if (_currentSelectMode == SelectMode.Add)
                                {
                                    ...
                                    {
                                        Debug.LogError("BuildHelperWindow+Version DrawChannel error: duplicate channel=" + _currentInputEnumString);
                                    }
                                    else
                                    {
                    ...
                    default: Debug.LogError("BuildHelperWindow+Version DrawChannel error: unsupport channel select mode=" + _currentSelectMode); break;
                }
            }
