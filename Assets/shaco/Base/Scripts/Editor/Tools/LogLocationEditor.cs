...
namespace shacoEditor
{
    public static class LogLocationEditor
    {
        ...
        static private string GetCurrentFullFileName(string[] fileNames)
        {
            ...
            var shacoFrameworkPath = defaultPathAssets.ContactPath(shaco.Base.GlobalParams.GetShacoFrameworkRootPath().RemoveFront(defaultPathAssets));

            for (int i = 0; i < fileNames.Length; ++i)
            {
                var fileNameTmp = fileNames[i];

                //获取普通堆栈信息
                var fileNameAndLine = fileNameTmp.Substring(defaultPathAssets, ")");

                //如果是异常堆栈信息，是以空格结尾
                if (string.IsNullOrEmpty(fileNameAndLine))
                {
                    fileNameAndLine = fileNameTmp.Substring(defaultPathAssets, " ");
                }
                
                //如果是c#底层堆栈信息，是以第二个冒号结尾
                if (string.IsNullOrEmpty(fileNameAndLine))
                {
                    var firstColonIndex = fileNameTmp.IndexOf(":");
                    var lastColonIndex = fileNameTmp.LastIndexOf(":");
                    if (firstColonIndex != lastColonIndex)
                    {
                        var findStartIndex = fileNameTmp.IndexOf(defaultPathAssets);
                        if (findStartIndex >= 0)
                        {
                            findStartIndex += defaultPathAssets.Length;
                            fileNameAndLine = fileNameTmp.Substring(findStartIndex, lastColonIndex - findStartIndex);
                        }
                    }
                }
                ...
                if (!string.IsNullOrEmpty(fileNameAndLine))
                {
                    ...
                    if (!subFileName.Contains(shacoFrameworkPath) || fileNameTmp.Contains(shaco.Base.GlobalParams.FORCE_LOG_LOCATION_TAG))
                    {
