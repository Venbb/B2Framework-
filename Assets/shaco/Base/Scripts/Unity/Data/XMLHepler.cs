...
namespace shaco
{
    [System.Obsolete("Use shaco.ExcelData instead, This class may be removed in a future release")]
    public class XMLHelper
    {
        ...
        private bool LoadXML(string xmlFilePath)
        {
            ...
            if (!shaco.Base.FileHelper.ExistsFile(xmlFilePath))
            {
               Log.Error("XMLHelper LoadXML error: fullpath does not exist ! " + xmlFilePath);
               return false;
            }
            ...
            try
            {
                var readStrng = shaco.Base.FileHelper.ReadAllByUserPath(xmlFilePath);
                doc.LoadXml(readStrng);
            }
        ...
        static public void DeleteXml(string xmlFilePath)
        {
            if (shaco.Base.FileHelper.ExistsFile(xmlFilePath))
            {
                shaco.Base.FileHelper.DeleteByUserPath(xmlFilePath);
            }
            else
				Log.Error("DeleteXml error: not find xml by path = " + xmlFilePath);
        }
        ...
        static private void SaveXml(XmlDocument document, string xmlFilePath)
        {
            ...
            try
            {
				...
				Log.Info("Saved Xml file success: path = " + xmlFilePath);
            }
            catch (System.Exception ex)
            {
        ...
        static private XmlElement NewElement(XmlDocument document, string keyPath)
        {
			...
			var lastName = shaco.Base.FileHelper.GetLastFileName(pathTmp, shaco.Base.FileDefine.PATH_FLAG_SPLIT, false);
            ...
            do
            {
                int flagIndex = pathTmp.LastIndexOf(shaco.Base.FileDefine.PATH_FLAG_SPLIT);
                if (flagIndex == -1)
                {
                    break;
                }
                ...
                {
					lastName = shaco.Base.FileHelper.GetLastFileName(pathTmp, shaco.Base.FileDefine.PATH_FLAG_SPLIT, false);
