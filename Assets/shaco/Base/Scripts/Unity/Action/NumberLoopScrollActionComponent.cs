...
namespace shaco
{
    ...
    public class NumberLoopScrollActionComponent : MonoBehaviour
    {
        //滚动方向
        ...
        //滚动数字模板
        [SerializeField]
        private Text _model = null;

        //每个数组模板滚动流畅度
        [SerializeField]
        [Range(1, 10)]
        private int _smooth = 5;
        ...
        //所有的滚动动画组建
        private List<LoopScrollActionComponent> _scrollActions = new List<LoopScrollActionComponent>();
        ...
        private IEnumerator DelayAddScrolCallBack(LoopScrollActionComponent scrollAction, int index, string numberString)
        {
        ...
        private bool UpdateTextLayout(string NumberLoopScrollAction)
        {
            if (null == _model)
            {
                shaco.Log.Error("NumberLoopScrollAction UpdateTextLayout error: no text model", this);
                return false;
            }

            if (_currentPauseIndex >= _currentChangedIndex)
            {
                shaco.Log.Warning("NumberLoopScrollAction UpdateTextLayout warning: please wait scrolling", this);
                return false;
            }

#if UNITY_EDITOR
            if (FindParent(this.gameObject, _model.gameObject) != null)
            {
                shaco.Log.Error("NumberLoopScrollAction UpdateTextLayout error: text model can't be a parent of me", this);
                return false;
            }
#endif
            //重新计算需要更新的数字位
        ...
        private void CheckLayoutGroup(string numberString)
        {
            ...
            switch (scrollDirection)
            {
                ...
                default: shaco.Log.Error("NumberLoopScrollAction CheckLayoutGroup error: unsupport type=" + scrollDirection, this); break;
            }

            if (null == _rectTransform)
                _rectTransform = this.GetComponent<RectTransform>();
            _rectTransform.sizeDelta = new Vector3(newSizeTmp.x, _rectTransform.sizeDelta.y);
        }
        ...
        private char GetNumberStringInScrollAction(LoopScrollActionComponent action)
        {
            return action.GetScrollTarget(_displayIndex).GetComponent<Text>().text.ToChar();
        }
        ...
        private void AddScrollTextModel(char txt)
        {
            ...
            var newScrollAction = newActionTarget.AddComponent<LoopScrollActionComponent>();
        ...
        private void SetDisplayText(LoopScrollActionComponent scrollAction, string txt)
        {
            if (scrollAction.transform.childCount == 0)
                return;

            int safeDisplayIndex = _displayIndex;
            if (safeDisplayIndex < 0 || safeDisplayIndex > scrollAction.transform.childCount)
            {
                shaco.Log.Error("NumberLoopScrollAction AddScrollTextModel error: display index out of range, index=" + safeDisplayIndex + " count=" + scrollAction.transform.childCount, this);
                safeDisplayIndex = 0;
            }
        ...
        private void ResetNumberTextScrollActions()
        {
            ...
            for (int i = _scrollActions.Count - 1; i >= _currentChangedIndex; --i)
            {
                try
                {
                    var childGameObject = _scrollActions[i].gameObject;
                    if (childGameObject != _model.gameObject)
                        UnityHelper.SafeDestroy(childGameObject);
                }
        ...
        private Text AddTextByModel(LoopScrollActionComponent scrollAction, string txt)
        {
            var newText = ((GameObject)(MonoBehaviour.Instantiate(_model.gameObject))).GetComponent<Text>();
