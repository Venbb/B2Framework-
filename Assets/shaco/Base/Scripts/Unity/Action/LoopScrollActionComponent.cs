...
namespace shaco
{
    ...
    public class LoopScrollActionComponent : MonoBehaviour
    {
        //滚动方向
        ...
        //<滚动偏移量(本地坐标)，滚动时间>
        //返回动画对象
        public System.Func<Vector3, float, ActionBase> delegateScrollAction;
        ...
        public bool RunScrollAction(int scrollTimes = 0, bool autoUpdateLayout = true)
        {
            ...
            {
                shaco.Log.Error("LoopScrollAction RunScrollAction error: no action target", this);
                return false;
            }

            if (_isScrolling)
            {
                shaco.Log.Warning("LoopScrollAction RunScrollAction warning: scrolling, please wait...", this);
                return false;
            }
            _isScrolling = true;
            ...
            {
                shaco.Log.Error("LoopScrollAction RunScrollAction error: first action target missing 'RectTransform'", this);
                return false;
            }
        ...
        public RectTransform GetScrollTarget(int index)
        {
            ...
            {
                shaco.Log.Error("LoopScrollAction GetScrollTarget error: out of range, index=" + index + " count=" + scrollTargets.Count, this);
                return retValue;
            }
        ...
        private void UpdateActionTargets()
        {
            ...
            foreach (var child in this.transform)
            {
                ...
                if (rectTransTmp == null)
                {
                    shaco.Log.Error("LoopScrollAction CheckLayoutGroup error: not have 'RectTransform' Component, target=" + child, this);
                }
                else
                {
        ...
        private void RunScrollActionLoop(shaco.Direction scrollDirection, float duration, int currentMoveTims, int scrollTimes)
        {
            ...
            for (int i = 0; i < scrollTargets.Count; ++i)
            {
                ...
                moveActionTmp.SetTag(MOVE_ACTION_TAG);
            }

            if (null != moveActionTmp)
            {
                moveActionTmp.onCompleteFunc += (ActionBase ac) =>
                {
                    ...
                    if (scrollTargets.Count > 1 && scrollTimes == 0)
                    {
                        ...
                        switch (scrollDirection)
                        {
                            ...
                            case shaco.Direction.Down: pivotGetTmp = shaco.Pivot.UpperCenter; pivotSetTmp = shaco.Pivot.LowerCenter; break;
                            case shaco.Direction.Left: break;
                            case shaco.Direction.Right: break;
                            default: shaco.Log.Error("LoopScrollAction RunScrollActionLoop error: unsupport direction=" + scrollDirection, this); break;
                        }
        ...
        private Vector3 GetScrollDelta(shaco.Direction scrollDirection)
        {
            ...
            switch (scrollDirection)
            {
                ...
                default: shaco.Log.Error("LoopScrollAction GetScrollDelta error: unsupport direction=" + scrollDirection, this); break;
            }
            return retValue;
        }
        ...
        private void CheckLayoutGroup(shaco.Direction scrollDirection)
        {
            ...
            if (null != layoutGroup)
            {
                ...
                switch (scrollDirection)
                {
                    ...
                    default: shaco.Log.Error("LoopScrollAction CheckLayoutGroup error: unsupport direction=" + scrollDirection, this); break;
                }
            }
