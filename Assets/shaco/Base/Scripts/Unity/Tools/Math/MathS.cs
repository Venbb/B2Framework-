...
namespace shaco
{
    public class MathS : shaco.Base.MathS
    {
		...
		private static float Cross(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4)
		{
			return (p2.x - p1.x) * (p4.y - p3.y) - (p2.y - p1.y) * (p4.x - p3.x);
		}
        private static float Area(Vector3 p1, Vector3 p2, Vector3 p3)
		{
			return Cross(p1, p2, p1, p3);
		}
        private static float fArea(Vector3 p1, Vector3 p2, Vector3 p3)
		{
			return Mathf.Abs(Area(p1, p2, p3));
		}
	
		private static double Mult(Vector3 a, Vector3 b, Vector3 c)
		{
