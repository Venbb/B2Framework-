...
namespace shaco.Test
{
    ...
    public class TestUI_3 : MonoBehaviour, shaco.IUIAnimation
    {
        ...
        void OnUIPreLoad()
        {
            shaco.Log.Info("TestUI_3 OnUIPreLoad", this);
        }
        ...
        void OnUIInit(TestArg arg)
        {
            ...
            shaco.Log.Info("TestUI_3 OnUIInit====" + testArg.message + " name=" + this.name + " index=" + (uiIndexMe), this);
            this.name = uiIndexMe.ToString();
        }

        void OnUIOpen(TestArg arg)
        {
            shaco.Log.Info("TestUI_3 OnUIOpen====" + arg.message + " name=" + this.name + " index=" + uiIndexMe, this);
        }

        void OnUIHide()
        {
            //挂载UILayerFullScreen作为全屏ui后，当一个全屏ui打开时候会自动隐藏最上层全屏属性ui，这个时候的arg是空的，所有需要判断下
            shaco.Log.Info("TestUI_3 OnUIHide", this);
        }

        void OnUIResume()
        {
            //挂载UILayerFullScreen作为全屏ui后，当一个全屏ui隐藏或者关闭时候会自动打开最上层全屏属性ui，这个时候的arg是空的，所有需要判断下
            shaco.Log.Info("TestUI_3 OnUIResume", this);
        }

        void OnUIClose()
        {
            shaco.Log.Info("TestUI_3 OnUIClose", this);
        }

        void OnUIRefresh(TestArg arg)
        {
            shaco.Log.Info("TestUI_3 OnUIRefresh====" + arg.message + " name=" + this.name + " index=" + uiIndexMe, this);
        }

        void OnUIBringToFront()
        {
            //挂载UILayerFullScreen作为全屏ui后，当一个全屏ui隐藏或者关闭时候会自动打开最上层全屏属性ui，这个时候的arg是空的，所有需要判断下
            shaco.Log.Info("TestUI_3 OnUIBringToFront", this);
        }

        public void OnClickHideMe()
        {
            //用下面的方法更加优雅
            // shaco.GameHelper.ui.HideUITarget<TestUI_3>(this, new TestArg());
            this.HideMe();
        }
        ...
        {
            //用下面的方法更加优雅
            // shaco.GameHelper.ui.CloseUITarget<TestUI_3>(this, new TestArg());
            this.CloseMe();
        }
