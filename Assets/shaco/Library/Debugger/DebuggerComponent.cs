...
namespace UnityGameFramework.Runtime
{
    ...
    public partial class DebuggerComponent : MonoBehaviour
    {
        /// <summary>
        /// 默认调试器漂浮框大小。
        /// </summary>
        ...
        /// 窗口位置保存key
        /// </summary>
        internal static string WINDOW_SAVED_POSITION_KEY = "DebuggerComponent.DefaultIconRect.position";
        ...
        private CloseWindow m_CloseWindow = new CloseWindow();
        ...
        private AssetBundleWindow m_RuntimeAssetbundleWindow = new AssetBundleWindow();
        private EventWindow m_EventWindow = new EventWindow();
        private ObjectPoolWindow m_RuntimeObjectPoolWindow = new ObjectPoolWindow();
        ...
        void Awake()
        {
            ...
            {
                shaco.Log.Exception("Debugger manager is invalid.");
                return;
            }
        ...
        private void Start()
        {
            RegisterDebuggerWindow("Close", m_CloseWindow);
            ...
            RegisterDebuggerWindow("Profiler/AssetBundle", m_RuntimeAssetbundleWindow);
            RegisterDebuggerWindow("Profiler/Event", m_EventWindow);
            RegisterDebuggerWindow("Profiler/ObjectPool", m_RuntimeObjectPoolWindow);
        ...
        private void OnGUI()
        {
            ...
            {
                var rectTmp = GUILayout.Window(m_DebuggerWindowID, m_IconRect, DrawDebuggerWindowIcon, "DEBUG");
                if (rectTmp != m_IconRect)
                {
                    rectTmp = RestrictInScreen(rectTmp);

                    //记录当前小窗口位置，在下次reset位置时候使用
                    m_IconRect = rectTmp;
                    shaco.GameHelper.datasave.WriteVector2(WINDOW_SAVED_POSITION_KEY, IconRect.position);
                }
            }
        ...
        private void DrawDebuggerWindowGroup(IDebuggerWindowGroup debuggerWindowGroup)
        {
            ...
            if (debuggerWindowGroup.SelectedIndex != toolbarIndex)
            {
                ...
                if (debuggerWindowGroup.SelectedWindow == m_CloseWindow)
                {
        ...
        private void SetShowFullWindow(bool isShow)
        {
            ...
            //默认选择第二个按钮(日志)
            if (null != m_DebuggerManager && null != m_DebuggerManager.DebuggerWindowRoot)
                m_DebuggerManager.DebuggerWindowRoot.SelectedIndex = 1;
        }
        ...
        private void ResetDebuggerWindowRect()
        {
            ...
            //默认为屏幕右下角
            var defaultPosition = new Vector2(Screen.width - DefaultIconRect.width, Screen.height - DefaultIconRect.height);
            DefaultIconRect.position = shaco.GameHelper.datasave.ReadVector2(WINDOW_SAVED_POSITION_KEY, defaultPosition);

            //显示窗口在合理范围内
            DefaultIconRect = RestrictInScreen(DefaultIconRect);
        ...
        private Rect RestrictInScreen(Rect rect)
        {
            var defaultPosition = new Vector2(Screen.width - rect.width, Screen.height - rect.height);

            if (rect.position.x < 0)
                rect.position = new Vector2(0, rect.position.y);
            if (rect.position.x > defaultPosition.x)
                rect.position = new Vector2(defaultPosition.x, rect.position.y);
            if (rect.position.y < 0)
                rect.position = new Vector2(rect.position.x, 0);
            if (rect.position.y > defaultPosition.y)
                rect.position = new Vector2(rect.position.x, defaultPosition.y);

            return rect;
        }

        //add by shaco 2020/04/01
        /// <summary>
        /// 初始化框架运行环境，如果是Unity5.3以下的版本，请在初始场景中手动添加DebuggerComponnet
        /// </summary>
#if UNITY_5_3_OR_NEWER
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]
#endif
        static private void InitFrameworkEnvironment()
        {
#if DEBUG_WINDOW
            bool hasOutLogScript = shaco.GameEntry.GetComponentInstance<shaco.SceneManager>().GetComponent<UnityGameFramework.Runtime.DebuggerComponent>() != null;
            if (!hasOutLogScript)
            {
                var target = shaco.GameEntry.GetComponentInstance<shaco.SceneManager>().gameObject;
