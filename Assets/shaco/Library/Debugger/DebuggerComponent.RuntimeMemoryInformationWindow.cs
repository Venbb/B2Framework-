        ...
        private sealed partial class RuntimeMemoryInformationWindow<T> : ScrollableDebuggerWindowBase where T : UnityEngine.Object
        {
            ...
            protected override void OnDrawScrollableWindow(Rect lastGUIRect)
            {
                ...
                GUILayout.BeginVertical("box");
                {
                    ...
                    {
                        if (GUILayout.Button(string.Format("Take Sample for {0}", typeName), GUILayout.Width(DefaultWindowRect.width / 2), GUILayout.Height(DefaultWindowRect.height / 16)))
                        {
                            TakeSample();
                        }
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < m_Samples.Count; i++)
                        {
                            ...
                            {
                                if (!m_Samples[i].Name.ToLower().Contains(_searcNameLower))
                                    continue;
                            }
