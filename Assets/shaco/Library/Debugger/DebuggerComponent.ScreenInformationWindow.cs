        ...
        private sealed class ScreenInformationWindow : ScrollableDebuggerWindowBase
        {
            protected override void OnDrawScrollableWindow(Rect lastGUIRect)
            {
                ...
                GUILayout.BeginVertical("box");
                {
                    ...
                    DrawItem("Screen Width", string.Format("{0} px / {1} in / {2} cm", Screen.width.ToString(), GameFramework.Utility.Converter.GetInchesFromPixels(Screen.width).ToString("F2"), GameFramework.Utility.Converter.GetCentimetersFromPixels(Screen.width).ToString("F2")));
                    DrawItem("Screen Height", string.Format("{0} px / {1} in / {2} cm", Screen.height.ToString(), GameFramework.Utility.Converter.GetInchesFromPixels(Screen.height).ToString("F2"), GameFramework.Utility.Converter.GetCentimetersFromPixels(Screen.height).ToString("F2")));
