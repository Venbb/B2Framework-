...
namespace UnityGameFramework.Runtime
{
    ...
    public static class Helper
    {
        ...
        public static T CreateHelper<T>(string helperTypeName, T customHelper, int index) where T : MonoBehaviour
        {
            ...
            {
                System.Type helperType = GameFramework.Utility.Assembly.GetTypeWithinLoadedAssemblies(helperTypeName);
                if (helperType == null)
                {
                    shaco.Log.WarningFormat("Can not find helper type '{0}'.", helperTypeName);
                    return null;
                }

                if (!typeof(T).IsAssignableFrom(helperType))
                {
                    shaco.Log.WarningFormat("Type '{0}' is not assignable from '{1}'.", typeof(T).FullName, helperType.FullName);
                    return null;
                }
            ...
            {
                shaco.Log.WarningFormat("You must set custom helper with '{0}' type first.", typeof(T).FullName);
                return null;
            }
            ...
            {
                helper = Object.Instantiate(customHelper) as T;
            }
