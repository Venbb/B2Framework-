...
namespace shaco.Instance.RichText
{
    public partial class GUILayoutHelper
    {
        ...
        {
            if (null == readonlyAnchors || readonlyAnchors.Length == 0)
        ...
        static private TextAnchor DrawAnchorGroup(TextAnchor anchor, int groupIndex, string[] groupAnchorName, TextAnchor[] readonlyAnchors, params GUILayoutOption[] options)
        {
            ...
            var newSelectIndex = GUILayout.Toolbar(selectIndex, groupAnchorName, options);
            if (newSelectIndex != -1 && newSelectIndex != selectIndex)
            {
