    ...
    public partial class Assets
    {
        ...
        internal static void OnLoadManifest(BundleManifest manifest)
        {
            ...
            for (int i = 0, max = assets.Length; i < max; i++)
            {
                ...
                {
                    Debug.LogError(string.Format("{0} bundle {1} not exist.", path, item.fileName));
                }
            }
        }
        ...
        private static void UpdateBundles()
        {
            ...
            for (var i = 0; i < _unusedBundles.Count; i++)
            {
                ...
                if (request.isDone)
                {
                    ...
                    request.Unload();
                    Debug.Log("UnloadBundle: " + request.url);
        ...
        internal static BundleRequest LoadBundle(string assetBundleName, bool async)
        {
            ...
            var url = Utility.Assets.GetDataPath(assetBundleName);
        ...
        internal static string RemapVariantName(string assetBundleName)
        {
            ...
            if (bestFit == int.MaxValue - 1)
                Debug.LogWarning("Ambiguous asset bundle variant chosen because there was no matching active variant: " + bundlesWithVariant[bestFitIndex]);
