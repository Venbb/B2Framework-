...
namespace B2Framework
{
    ...
    public class Updater : MonoBehaviour, IUpdater, INetworkListener
    {
        ...
        public void StartUpdate()
        {
            ...
            Utility.Assets.CreatePersistentDataDir();
        ...
        private void RequestVersions()
        {
            ...
            var url = Utility.Assets.GetDownloadURL(AppConst.RES_VER_FILE);
            Debug.Log(url);
                     ...
                     try
                     {
                         ...
                         if (newFiles.Count > 0)
                         {
                             foreach (var item in newFiles)
                             {
                                 ...
                                 _downloader.AddDownload(Utility.Assets.GetDownloadURL(item.name), item.name, path, item.hash, item.len);
                             }
        ...
        private void CheckVersions()
        {
            ...
            if (totalSize > 0)
            {
                var tips = string.Format("发现内容更新，总计需要下载 {0} 内容", Utility.FormatSize(totalSize));
        ...
        {
            OnMessage(Utility.Text.Format("下载中...{0}/{1}, 速度：{2}", Utility.FormatSize(progress), Utility.FormatSize(size), Utility.FormatSize(speed, true)));
        ...
        {
            OnMessage("正在初始化");
            var init = Assets.Instance.Initialize();
            yield return init;
        ...
        {
            return Utility.Assets.GetDataPath(path);
        }
