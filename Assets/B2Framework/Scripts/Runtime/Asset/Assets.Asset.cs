    ...
    public partial class Assets
    {
        ...
        private static void UpdateAssets()
        {
            ...
            for (var i = 0; i < _unusedAssets.Count; ++i)
            {
                ...
                Debug.Log(string.Format("UnloadAsset:{0}", request.url));
                request.Unload();
            ...
            for (int i = 0; i < _scenes.Count; ++i)
            {
                ...
                _scenes.RemoveAt(i);
                Debug.Log(Utility.Text.Format("UnloadScene:{0}", request.url));
                request.Unload();
                UnloadUnusedAssets();
                --i;
            }
        ...
        internal static AssetRequest LoadAsset(string path, Type type, bool async)
        {
            ...
            {
                if (Utility.Assets.IsRemote(path))
            ...
            request.Retain();
            Debug.Log(Utility.Text.Format("LoadAsset:{0}", path));
            return request;
        }
