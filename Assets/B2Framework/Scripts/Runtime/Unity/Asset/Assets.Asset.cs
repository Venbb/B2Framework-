    ...
    public partial class Assets
    {
        ...
        private static void UpdateAssets()
        {
            ...
            for (var i = 0; i < _unusedAssets.Count; ++i)
            {
                ...
                Log.Debug(string.Format("UnloadAsset:{0}", request.url));
                request.Unload();
            ...
            for (int i = 0; i < _scenes.Count; ++i)
            {
                ...
                _scenes.RemoveAt(i);
                Log.Debug(Utility.Text.Format("UnloadScene:{0}", request.url));
                request.Unload();
                UnloadUnusedAssets();
                --i;
            }
        ...
        internal static AssetRequest LoadAsset(string path, Type type, bool async)
        {
            ...
            request.Retain();
            Log.Debug(Utility.Text.Format("LoadAsset:{0}", path));
            return request;
        }
