...
namespace B2Framework.Unity
{
    public class LuaManager : MonoSingleton<LuaManager>
    {
        ...
        private static byte[] CustomLoader(ref string filepath)
        {
            ...
            {
                scriptPath = Utility.Path.Combine(AppConst.LUA_SCRIPTS_PATH, filepath) + ".lua"; ;

                return Utility.Files.ReadAllBytes(scriptPath);
            }

            scriptPath = GameUtility.Assets.GetAssetPath(AssetBundles.Lua, filepath) + AppConst.LUA_EXTENSION;
